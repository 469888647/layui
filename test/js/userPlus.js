...
layui.define(['jquery', 'layer', 'table' , 'laypage' , 'form', 'laydate', 'dropdown'], function (exports) {
    ...
    const cols = [
        [
            ...
            },
            {
                 ...
                 * 这里使用使用模板的方式完成编辑
                 */
                templet: '#UserPlus-edit-userrole',
                minWidth: 160,
            },
            {
                field: "createtime",
                 ...
                 * 这里使用使用模板的方式完成编辑
                 */
                templet: '#UserPlus-edit-createtime',
                minWidth: 120,
            },
            {
                field: "updatetime",
                 ...
                 * 这里使用使用模板的方式完成编辑
                 */
                templet: '#UserPlus-edit-updatetime',
                minWidth: 120,
            },
            {
                field: "isdelete",
                 ...
                 * 这里使用使用模板的方式完成编辑
                 */
                templet: '#UserPlus-edit-isdelete',
                minWidth: 120,
            },
            {
                fixed: "right",
                title: "操作",
                toolbar: "#UserPlus-table-toolbar",
                width: 130,
            },
        ],
    ];
    ...
    const URL = LAYER_IFRAME_URL + "/user" + "/pageUser";
    const UPDATE_URL = LAYER_IFRAME_URL + "/user" + "/updateUser";

    const DELETE_URL = LAYER_IFRAME_URL + "/user" + "/deleteUser";
    ...
    let handler = {
        ...
        searchTable: function(flag = true){
            let param = {
                // url: URL,
                url: "../plugin/layuiframework/json/table/userdemo.json",
                // method: "post",
        ...
        addListener: function(){
            ...
            $("#" + ID + "-add").off("click").on("click", function(){
                ...
                // handler.parentLayui.openPopup("user" + "Add", { key: KEY});
                // 以frame管理的选项卡式iframe页面方式打开(打开的是iframe)
                handler.parentLayui.frame.setTop("user" + "IframeAdd", { key: KEY});
            });
            /**
             *  表格操作栏点击事件集合
             */
            handler.tableEvent = {
                ...
                "update": function(data){
                    ...
                    // 传入key让添加页面能够知道是哪个页面跳转过来的
                    // 以layui抽屉弹层的方式弹出(弹出的是窗口层)
                    // handler.parentLayui.openPopup("user" + "Update", { data: data[0], key: KEY });
                    // 以frame管理的选项卡式iframe页面方式打开(打开的是iframe)
                    handler.parentLayui.frame.setTop("user" + "IframeUpdate", { data: data[0], key: KEY });
                },
                /**
                 *  表格头部工具栏上面的删除按钮点击事件
                 */
                ...
                "deleteByPid": function(obj){
                    ...
                    // 获取到这一行上面的数据信息
                    let data = obj.data;
                    let _objName = data['useraccount']
                    layui.layer.confirm(
                        _objName ? "您确认删除用户: " + _objName + " 吗？" : "您确认删除吗？",
                        function (index) {
                            layui.layer.close(index);
