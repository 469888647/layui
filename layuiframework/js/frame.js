 ...
 */
("use strict");
layui.define(["jquery", "element", "layer", "util","dropdown","form","table"], function (exports) {
  // 初始化jQuery
  ...
  let handler = {
     ...
     * @inner iframe配置项缓存
     * @desc
     *    <p style = "color: #16b777;text-indent: 10px;">iframe配置项缓存:保存的类似于一个key - value形式的map</p>
     *    <ul>
     *      <li>key - 可以是点击页面的一个id(可能是字符串)</li>
     *      <li>value - 值的可以是一个不断提升的数字,和页面上面的tab页面控制的id和iframe区域的div的属性挂钩</li>
     *    </ul>
     */
    cache: new lruCache(),
    /**
    ...
    /**
     * @inner  执行窗口的resize回调函数
     * @method 执行已缓存的窗口key的resize回调函数
     * @param {*} key 窗口的id,就是菜单调用时的key
     */
    ...
    /**
     * @inner 移除窗口的resize回调函数
     * @method 为key代表的窗口取消窗口resize事件
     * @param {*} key 窗口的id,就是菜单调用时的key
     */
    ...
    /**
     * @inner  移动index到可视的区域
     * @method  将当前编号index的tab栏移动到可视的区域
     * @param {*} index tab的编号
     */
    ...
    openTabsPage: function (key, data) {
      ...
      // 这里等待执行iframe的主方法(run) - 这里不是openlayer的方式打开的,需要手动触发回调函数
    ...
    /**
     * @inner  iframe标题的右键点击事件
     * @method 添加iframe标题的右键点击事件
     * @param key    窗口的id,就是菜单调用时的key
     * @param tabDom iframe标题的dom对象
     */
    ...
    /**
     * @inner  关闭所有的iframe页面
     * @method 关闭并删除除所有的iframe页面
     */
    closeAllPage: function () {
      _lodash.each(handler.cache.cache, (v, k) => handler.closeTabsPage(k));
    },
    /**
     * @inner  通过方法名调用iframe的方法
     * @method  根据方法名称调用key代表的iframe页面的layui模块下面的方法
     * @param {*} key 窗口的id,就是菜单调用时的key
    ...
    /**
     * @inner  在iframe就绪后执行回调
     * @method  在key代表的iframe页面的layui模块加载完毕后执行回调
     * @param {*} key 窗口的id,就是菜单调用时的key
    ...
    /**
     * @inner  在条件就绪为true后执行回调
     * @method  条件变为true时执行回调函数
    ...
    addListener: function () {
      ...
      layui.util.event("lay-header-event", {
        ...
        refresh: function () {
          ...
          var option =
            iframe[0].contentWindow.layui[key] &&
            iframe[0].contentWindow.layui[key].option;
      ...
      setTimeout(function () {
        ...
        // 初始化主题设置
        layui.colortheme && layui.colortheme.run();
      }, 200);
    },
  };
  // 调用添加事件
  ...
  // 主题监听事件修改,对colortheme进行扩展,试主页面和iframe页面的主题一致
